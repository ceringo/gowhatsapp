            ...
            if(!userRules[msg.from] || userRules[msg.from].length === 0) {
             ...
             if(message.length > 0) {
                ...
                if(message[0].call == "true") {
                    const number =  await (await msg.getContact()).number;
                    const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
                    + 'VERSION:3.0\n' 
                    + `FN:${nombreContacto}-${number}\n` // full name
                    + 'ORG:;\n' // the organization of the contact
                    + `TEL;type=CELL;type=VOICE;waid=${number}:+${number}\n` // WhatsApp ID + phone number
                    + 'END:VCARD'
                    await client.sendMessage(phoneNumberFormatter(businessNumber),vcard, { parseVCards: true });
...
app.post('/loadRules', async (req, res) => {
    const { data, business,nameVcard } = req.body;
    businessNumber = business;
    nombreContacto= nameVcard;
